...
export default class ServicesUp extends BaseCommand {
  static args = {
    ...BaseCommand.args,
    name: Args.string({ description: "Container name or Stack name in swarm mode" }),
  };
  ...
  static flags = {
    ...BaseCommand.flags,
    build: Flags.boolean({ char: "b", description: "Build images", default: false }),
    init: Flags.boolean({ char: "i", description: "Init docker swarm (only swarm mode)", default: false })
  };
  ...
  public async run(): Promise<ProcessOutput> {
    ...
    if (flags.swarm) {
      ...
      if (flags.env === "dev") {
        commandArgs.push("-d", "--build", args.name || "");
      }

...
try:
    ...
    os.system("pip3 install --user -r ./cli/requirements.txt")
...
@click.option(
    "--disable-telemetry",
    envvar="MLCRAFT_DISABLE_TELEMETRY",
    is_flag=True,
    default=False,
)
@click.option(
    "--telemetry-url",
    envvar="MLCRAFT_TELEMETRY_URL",
    default="https://api.mlcraft.org/v1/graphql",
)
@click.pass_context
def main(ctx, env, config_file, disable_telemetry, telemetry_url):
    ...
    if not config:
        config = {
            "user": {"anonymous_id": os.environ.get("ANONYMOUS_ID", str(uuid.uuid4()))}
        }
    ...
    if not disable_telemetry:
        ...
        create_cli_event(url, {"user": config.get("user", {})})
    else:

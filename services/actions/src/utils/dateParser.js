...
const momentFromResult = (result, timezone = DEFAULT_TIMEZONE) => {
...
const dateParser = (dateString, timezone = DEFAULT_TIMEZONE) => {
  let momentRange;
  dateString = dateString.toLowerCase();
  if (
    dateString.match(
      /(this|last)\s+(day|week|month|year|quarter|hour|minute|second)/
    )
  ) {
    ...
    dateString.match(
      /last\s+(\d+)\s+(day|week|month|year|quarter|hour|minute|second)/
    )
  ) {
    const match = dateString.match(
      /last\s+(\d+)\s+(day|week|month|year|quarter|hour|minute|second)/
    ...
    if (!toResults) {
      throw new Error(`Can't parse date: '${to}'`);
    }
    const exactGranularity =
      ["second", "minute", "hour"].find((g) => dateString.indexOf(g) !== -1) ||
      "day";
    momentRange = [
    ...
    momentRange = [
      momentRange[0].startOf(exactGranularity),
      momentRange[1].endOf(exactGranularity),
    ];
  } else {
